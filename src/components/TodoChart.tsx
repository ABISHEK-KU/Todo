...
export const TodoChart: React.FC<Props> = ({ todos = [] }) => {
  ...
  return (
    <div style={{ width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
      <div style={{ height: 300, background: "var(--card)", padding: 12, borderRadius: 8 }}>
        <h3 style={{ margin: "0 0 8px 0" }}>Status Distribution</h3>
        <ResponsiveContainer width="100%" height={220}>
          <PieChart>
            <Pie data={pieData} dataKey="value" nameKey="name" outerRadius={80} innerRadius={30} paddingAngle={4}>
              {pieData.map((entry) => (
      ...
      <div style={{ height: 300, background: "var(--card)", padding: 12, borderRadius: 8 }}>
        <h3 style={{ margin: "0 0 8px 0" }}>Tasks by Due Date</h3>
        <ResponsiveContainer width="100%" height={220}>
